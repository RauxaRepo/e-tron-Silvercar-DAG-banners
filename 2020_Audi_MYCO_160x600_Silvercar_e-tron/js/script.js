...
window.addEventListener("load", function () {
    ...
    if("2020_Audi_T2_AIS_DP_Interest_Atlanta_2020_A6_Generic") {
        var getInfo = "2020_Audi_T2_AIS_DP_Interest_Atlanta_2020_A6_Generic".split("_");
    ...
    lookupType = "".toLowerCase().split("_").join("").split(" ").join("").split("-").join("");
...
function preloadImage(f) {
    ...
    a = [
        "Silvercar_etron_160x600_fr1.jpg",
        z
    ];
...
function initializeUnit() {
    ...
    outer_backgroundTwo.src = "Silvercar_etron_160x600_fr2.jpg"
    if(isRetinaDisplay()){
    ...
    cta.innerHTML = "<span style='font-size:120%'>See offers ></span>";
    replay.innerHTML = "Replay";
    



    if ("|#fff||||62,570".split("|")[1] != "") s.innerHTML += ".cta { color: " + "|#fff||||62,570".split("|")[1] + "; }\n\n";
    if ("|#fff||||62,570".split("|")[5] != "") s.innerHTML += ".cta { left: " + "|#fff||||62,570".split("|")[5].split(",")[0] + "px; top: " + "|#fff||||62,570".split("|")[5].split(",")[1] + "px; }\n";
        ...
        if (!"https://www.audiusa.com/models/audi-e-tron") {
            ...
            if(isMLP){
                ...
                if(clickOut.indexOf('?') != -1){
                    ...
                    clickTagURL = clickOut + "?ddcref=t2_programmatic-display_PHDP_AO_" +region +"_" + market + "_x_" + funnel + "_" + modelYear + "_" + model.split('e-reon').join('').split('a5-sportback').join('a5') + "_PHD_x_x_2020-01-07";
                }
            }
...
function playAd(){
    normalizeHeadline();
    if("9.5|#fff|14,523" !=""){
        replay.style.fontSize = parseFloat("9.5|#fff|14,523".split("|")[0]) + "px";
        replay.style.left = parseFloat("9.5|#fff|14,523".split("|")[2].split(",")[0]) + "px";
        replay.style.top = parseFloat("9.5|#fff|14,523".split("|")[2].split(",")[1]) + "px";
        replay.style.color = "9.5|#fff|14,523".split("|")[1];
    }
    ...
    if("<span style='font-size:120%'><span style='color:#aaa'>View key offer details.</span></span>"){
...
function animateFrameTwo(){
    ...
    if(!disclaimerClick){
        TweenMax.to(inner_headline, .63, {opacity:0, y:300,rotate:.01, ease:Power3.easeInOut});
        TweenMax.to(inner, .63, {y:-300, ease:Power3.easeInOut, rotate:.01,
            onStart:function(){
            ...
            onComplete:function(){
                ...
                // TweenMax.set(inner_headline,{opacity:1});s
                inner.classList.remove('innerShadow');
                outer_backgroundOne.src = "Silvercar_etron_160x600_fr3.jpg";
                secondFrame = true;
...
function animateFrameThree(){
    setTimeout(function(){
        animateHeadline(function(){
            ...
            // TweenMax.set(disclaimer,{opacity:1});
            
            inner.classList.add('innerShadow');
            TweenMax.to(wrapper,.63,{height:"600px",ease:Power3.easeInOut, rotate:.01, onStart:function(){
                ...
                if(disclaimerThree){
                    ...
                    // disclaimer.removeAttribute("span");
                        disclaimer.innerHTML = "<span style='font-size:120%'><span style='color:#aaa'>View key offer details.</span></span>";
                        disclaimer.style.display = "block";
                        disclaimer.style.left = parseFloat("8|#fff|14,455".split("|")[2].split(",")[0]) + "px";
                        disclaimer.style.top = parseFloat("8|#fff|14,455".split("|")[2].split(",")[1]) + "px";
                        ...
                        disclaimer.addEventListener("click", function f(e){
                            overlay_content.style.fontSize = parseFloat("8|#fff|14,455".split("|")[0]) + "px";
                            overlay_content.style.lineHeight = parseFloat("8|#fff|14,455".split("|")[0]) + 1 + "px";
                            disclaimerEndGo();
                            disclaimerClick = true;
                        });
                    }
                TweenMax.to(inner, .63, {y:0, height:p, top:j + "px", ease:Power3.easeInOut, rotate:.01, onComplete:function(){
    ...
    inner_headline.addEventListener("transitionend", function f(e){
        ...
        if("" != "") {
            ...
            var l = document.createElement("img");
                ...
                l.src = "Silvercar_etron_160x600_ef.png";
...
function resetAll() {
    TweenMax.set(document.getElementsByClassName('headlineEnd')[0],{y:0});
    TweenMax.set(wrapper,{top:"0px",height:"200px"});
    main.removeChild(document.getElementsByClassName("headlineEnd")[0]);
...
function resizeHeadline(){
    ...
    if(a.indexOf("|") != -1){
        ...
        if(a_text[(a_text.length-1)].length > 10){
            copySize = "small";
        } else if(a_text[(a_text.length-1)].length >6){
            copySize = "medium";
        } else if(a_text[0].length <=6){
            copySize = "large"
        } else if((a_text[(a_text.length-2)].length) > 10){
            // console.log((a_text[(a_text.length-2)].length + 1));
            // console.log("here");
            copySize = "small";
        } else if((a_text[(a_text.length-2)].length) > 6){
            copySize = "medium";
        } else if((a_text[1].length) <= 6){
            copySize = "large";
        } 
    } else{
        var a_text = a.split(" ");
        if(a_text[(a_text.length-1)].length > 10){
            ...
            copySize = "small";
        } else if((a_text[(a_text.length-1)].length + a_text[(a_text.length-2)].length + 1) > 6){
            copySize = "medium";
        } else if((a_text[0].length + a_text[1].length + 1) <= 6){
            copySize = "large";
        } 
    }
    ...
    if(b.indexOf("|") != -1){
        ...
        if(b_text[(b_text.length-1)].length > 10){
            copySizeTwo = "small";
        } else if(b_text[(b_text.length-1)].length >6){
            copySizeTwo = "medium";
        } else if(b_text[0].length <=6){
            copySizeTwo = "large"
        } else if((b_text[(b_text.length-1)].length + b_text[(b_text.length-2)].length + 1) > 10){
            copySizeTwo = "small";
        } else if((b_text[(b_text.length-1)].length + b_text[(b_text.length-2)].length + 1) > 6){
            copySizeTwo = "medium";
        } else if((b_text[0].length + b_text[1].length + 1) <= 6){
            copySizeTwo = "large";
        } 
    } else{
        var b_text = b.split(" ");
    ...
    if(copySize == "small" || copySizeTwo == "small") {
        return "small";
    } else if(copySize == "medium" || copySizeTwo == "medium") {
        return "medium";
    } else if(copySize == "large" || copySizeTwo == "large") {
        return "large";
    }
        ...
        if(a.indexOf("|") != -1){
            ...
            for(var j=0;j<a_text.length;j++){
                ...
                if(c != undefined){
                    ...
                    if(copySize != "small" && copySize != "medium"){
                        g.style.fontSize = "24px";
                    } else if(copySize != "small"){
...
function normalizeHeadline(f) {
    ...
    if(secondFrame){
        inner_headline.style.top = 32 + (268 - inner_headline.getBoundingClientRect().height)/2 + "px";
        if ("|#fff|".split("|")[2] != "") {
        ...
        setTimeout(function(){
            TweenMax.set(inner_headline,{y:0});
            animateHeadline(function(){
              animateFrameThree();  
            });
        ...
        document.getElementsByClassName('headlineEnd').style.top = (97 - document.getElementsByClassName('headlineEnd')[0].getBoundingClientRect().height)/2 + "px";
        if ("".split("|")[2] != "") {
        ...
        inner_headline.style.top = (300 - inner_headline.getBoundingClientRect().height)/2 + "px";
        if ("|#fff|".split("|")[2] != "") {
            inner_headline.style.left =  "|#fff|".split("|")[2].split(",")[0] + "px";
            inner_headline.style.top =  "|#fff|".split("|")[2].split(",")[1] + "px";
        }
